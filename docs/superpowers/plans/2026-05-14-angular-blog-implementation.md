...
**Architecture:** Angular CLI 워크스페이스를 `app/` 서브폴더에 스캐폴드 후 `ng add @analogjs/platform` 으로 AnalogJS 추가. 위키 콘텐츠는 `app/src/content/` 로 이주. Tailwind v4 로 DESIGN.md 토큰을 매핑하고 remark 플러그인 체인으로 위키링크·글로서리·코드 하이라이트를 빌드 타임에 처리. `main` 브랜치 푸시 시 Cloudflare Pages 가 SSG 빌드 후 정적 호스팅.
...
.gitignore                                 # app/node_modules/, app/dist/, .DS_Store 추가
```
...
**Files:**
- Create: `app/` 전체 (Angular CLI 자동 생성)
...
Expected:
- `app/` 디렉토리가 새로 생성됨
- `app/package.json`, `app/angular.json`, `app/src/` 등 구조 생성
- `pnpm install` 자동 실행되어 `node_modules/` 가 채워짐

검증:
```bash
ls -la app/
cat app/package.json | grep "@angular/core"
```
...
```bash
git -C /home/hh4518/dev/hseongh-wiki add app/ .gitignore
...
**Files:**
- Modify: `app/angular.json` (자동)
- Modify: `app/vite.config.ts` (자동 생성)
- Modify: `app/package.json` (자동)

- [ ] **Step 1: @analogjs/platform schematic 실행**

Run:
```bash
cd /home/hh4518/dev/hseongh-wiki/app
...
```bash
git -C /home/hh4518/dev/hseongh-wiki add app/
...
- Create: `app/tailwind.config.ts` (v3 의 경우) 또는 `app/src/styles/theme.css` (v4 CSS-first)
- Modify: `app/angular.json` (styles 배열에 global.css 추가)
...
Edit `app/angular.json`, `projects.app.architect.build.options.styles` 배열에 `src/styles/global.css` 를 추가합니다 (기존 `src/styles.css` 옆에 또는 대체):
...
Edit `app/src/app/app.component.html` (또는 AnalogJS 의 동등한 파일), 임시로:
...
- Create: `app/src/styles/tokens-dark.css`
...
**Files:**
- Create: `app/public/fonts/` (woff2 파일들)
...
Edit `app/src/styles/global.css`, 상단에 추가:
...
`app/src/app/app.component.html` 에 임시 마크업 추가:
...
**Files:**
- Move: 루트의 `wiki/`, `_glossary/`, `_projects/`, `index.md`, `log.md`, `STYLE.md` → `app/src/content/`

- [ ] **Step 1: app/src/content/ 디렉토리 생성**

```bash
cd /home/hh4518/dev/hseongh-wiki
mkdir -p app/src/content/_meta
```

- [ ] **Step 2: 콘텐츠 파일 이동**

```bash
git mv wiki app/src/content/wiki
git mv _glossary app/src/content/_glossary
git mv _projects app/src/content/_projects
git mv index.md app/src/content/_meta/index.md
git mv log.md app/src/content/_meta/log.md
git mv STYLE.md app/src/content/_meta/STYLE.md
```

확인:
```bash
ls app/src/content/
ls app/src/content/_meta/
```
...
```bash
rm -f app/src/content/_glossary/.gitkeep
rm -f app/src/content/_projects/.gitkeep
rm -f app/src/content/wiki/.gitkeep
```

- [ ] **Step 4: 커밋**

```bash
git -C /home/hh4518/dev/hseongh-wiki add -A app/src/content/
git -C /home/hh4518/dev/hseongh-wiki commit -m "refactor: 위키 콘텐츠를 app/src/content/ 로 이주"
```
...
**Files:**
- Create: `app/src/content/_meta/domains.yml`
- Modify: `app/src/content/_projects/pnpm.md`
...
Edit `app/src/content/_projects/pnpm.md`, frontmatter 의 `last_ingest` 위에 추가:
...
```bash
git -C /home/hh4518/dev/hseongh-wiki add app/src/content/_meta/domains.yml app/src/content/_projects/pnpm.md
...
Edit `app/vite.config.ts`, `@analogjs/platform` 설정에 `content` 옵션을 추가합니다:
...
Edit `app/src/app/app.config.ts`:
...
```bash
git -C /home/hh4518/dev/hseongh-wiki add app/vite.config.ts app/src/app/app.config.ts
...
**Files:**
- Create: `app/src/lib/remark-wikilink.ts`
- Test: `app/src/lib/remark-wikilink.test.ts`
...
Create `app/src/lib/remark-wikilink.test.ts`:
...
```bash
git -C /home/hh4518/dev/hseongh-wiki add app/src/lib/remark-wikilink.ts app/src/lib/remark-wikilink.test.ts app/package.json app/pnpm-lock.yaml
...
**Files:**
- Create: `app/src/lib/shiki-config.ts`
- Modify: `app/vite.config.ts`
...
임시 stub `app/src/lib/build-lookups.ts`:
```typescript
...
**Files:**
- Create: `app/src/lib/build-lookups.ts`
- Test: `app/src/lib/build-lookups.test.ts`
...
Create `app/src/lib/build-lookups.test.ts`:
...
Replace `app/src/lib/build-lookups.ts` with:
...
```bash
git -C /home/hh4518/dev/hseongh-wiki add app/src/lib/build-lookups.ts app/src/lib/build-lookups.test.ts app/package.json app/pnpm-lock.yaml
...
**Files:**
- Create: `app/src/app/services/theme.service.ts`
- Test: `app/src/app/services/theme.service.spec.ts`
- Create: `app/src/app/components/theme-toggle/theme-toggle.component.ts`

- [ ] **Step 1: 실패하는 테스트 작성 (ThemeService)**

Create `app/src/app/services/theme.service.spec.ts`:
...
Edit `app/src/main.ts` (또는 app.component.ts 의 constructor):
...
```bash
git -C /home/hh4518/dev/hseongh-wiki add app/src/app/services/theme.service.ts app/src/app/services/theme.service.spec.ts app/src/app/components/theme-toggle/ app/src/main.ts app/src/app/app.component.ts
...
**Files:**
- Create: `app/src/app/components/header/header.component.ts`
...
Create `app/src/app/components/header/header.component.spec.ts`:
...
Edit `app/src/app/app.component.ts`:
...
```bash
git -C /home/hh4518/dev/hseongh-wiki add app/src/app/components/header/ app/src/app/app.component.ts
...
**Files:**
- Create: `app/src/app/components/footer/footer.component.ts`
...
Edit `app/src/app/app.component.ts`, `imports` 와 template 갱신:
...
```bash
pnpm dev   # 푸터 보이는지 확인
git -C /home/hh4518/dev/hseongh-wiki add app/src/app/components/footer/ app/src/app/app.component.ts
...
**Files:**
- Create: `app/src/app/components/badge/badge.component.ts`
- Create: `app/src/app/components/post-card/post-card.component.ts`
...
Create `app/src/app/components/post-card/post-card.component.spec.ts`:
...
```bash
git -C /home/hh4518/dev/hseongh-wiki add app/src/app/components/badge/ app/src/app/components/post-card/
...
**Files:**
- Create: `app/src/app/services/domain.service.ts`
- Create: `app/src/app/components/domain-sidebar/domain-sidebar.component.ts`
...
```bash
git -C /home/hh4518/dev/hseongh-wiki add app/src/app/services/domain.service.ts app/src/app/components/domain-sidebar/
...
**Files:**
- Create: `app/src/app/pages/(home).page.ts`
...
```bash
git -C /home/hh4518/dev/hseongh-wiki add app/src/app/pages/
...
**Files:**
- Create: `app/src/app/pages/wiki/[...path].page.ts`
- Create: `app/src/app/components/toc/toc.component.ts`
...
```bash
git -C /home/hh4518/dev/hseongh-wiki add app/src/app/pages/wiki/ app/src/app/components/toc/
...
**Files:**
- Create: `app/src/app/pages/projects/[slug].page.ts`
...
```bash
git -C /home/hh4518/dev/hseongh-wiki add app/src/app/pages/projects/
...
**Files:**
- Create: `app/src/app/pages/domains/[id].page.ts`
...
git -C /home/hh4518/dev/hseongh-wiki add app/src/app/pages/domains/
...
**Files:**
- Create: `app/src/app/pages/glossary/(index).page.ts`
- Create: `app/src/app/pages/glossary/[term].page.ts`
...
git -C /home/hh4518/dev/hseongh-wiki add app/src/app/pages/glossary/
...
**Files:**
- Create: `app/src/app/pages/about.page.ts`
...
```bash
pnpm dev   # /about 확인
git -C /home/hh4518/dev/hseongh-wiki add app/src/app/pages/about.page.ts
...
Edit `app/src/styles/global.css`, 기존 내용 아래에 추가:
...
git -C /home/hh4518/dev/hseongh-wiki add app/src/styles/global.css
...
```bash
git -C /home/hh4518/dev/hseongh-wiki add app/vite.config.ts
...
4. 빌드 설정:
   ...
   - **Build output directory**: `app/dist/analog/public`
   - **Root directory**: (비워둠)
...
- `wiki/<project>/<path>.md` → `app/src/content/wiki/<project>/<path>.md`
- `_glossary/<term>.md` → `app/src/content/_glossary/<term>.md`
- `_projects/<project>.md` → `app/src/content/_projects/<project>.md`
- `index.md`, `log.md`, `STYLE.md` 의 위치 → `app/src/content/_meta/` 하위
...
```bash
git -C /home/hh4518/dev/hseongh-wiki add README.md app/README.md
...
Edit `app/src/content/_meta/log.md`, 최상단에 추가:
...
```bash
git -C /home/hh4518/dev/hseongh-wiki add app/src/content/_meta/log.md
